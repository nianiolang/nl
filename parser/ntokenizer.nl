...
def eat_ws(ref state : @ntokenizer::state_t) {
	loop {
		...
		if(char == 9 || char == 13 || char == 32){
			...
			while(string::ord(get_char(ref state)) != 10 ){
				state->pos++;
				return if(state->pos == state->len);
			}
		}else{
...
def is_token(ref state : @ntokenizer::state_t) : @boolean_t::type {
	return true if ov::is(state->type, 'delimiter');
	return true if ov::is(state->type, 'operator');
	return true if ov::is(state->type, 'keyword');
	return false;
}
...
def ntokenizer::get_line(ref state : @ntokenizer::state_t) : ptd::sim() {
	return state->ln_nr;
}
...
def ntokenizer::info(ref state : @ntokenizer::state_t) : ptd::sim() {
...
def try_get_operator(state : @ntokenizer::state_t, ref char : ptd::sim()){
	fora var oper (get_char_oper()){
		...
		if(c_std_lib::fast_substr(state->text, state->pos, string::length(oper)) eq oper){
			continue if(string::is_letter(get_char(ref state)) && 
	...
	if (char eq '''') {
		...
		if(char eq '0' && get_char(ref state) eq 'x' && is_hex_number(get_next_char(ref state))){
			...
			loop {
				...
				break unless string::is_digit(char) || dot == 1;
				state->next_token .= char;
				state->pos++;
			}
		}

...
def flow_graph::state_t(){
	return ptd::rec({
		map => ptd::hash(@flow_graph::block_t),
		tab => ptd::arr(ptd::sim())
	});
}
...
def flow_graph::make_blocks(commands : ptd::arr(@nlasm::cmd_t), args_types : ptd::arr(@nlasm::arg_type_t)) : ptd::arr(@flow_graph::block_t) {
...
def add_block(ref blocks : @flow_graph::state_t, ref block : @flow_graph::block_t, nr : ptd::sim(), last_label : ptd::sim()) {
	block->to = nr;

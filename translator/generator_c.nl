...
def generator_c::global_const_t(){
	return ptd::rec({
		arr => ptd::arr(
			ptd::rec({key => ptd::sim(), uses => ptd::sim()})
		),
		hash => ptd::hash(ptd::sim()),
...
def generator_c::out_t() {
	return ptd::rec({
		modules => ptd::hash(@generator_c::module_out_t),
		global_const => @generator_c::module_out_t
	});
}

def generator_c::generate(nlasms : ptd::hash(@nlasm::result_t), ref state : @generator_c::state_t) : @generator_c::out_t {
	var modules_out = {};
	var modules_names : ptd::arr(ptd::sim()) = [];
...
def generate_global_const_files(ref state : @generator_c::state_t) : @generator_c::module_out_t {
	...
	println(ref state, '
		''.get_lib_fun('register_global_const') . '((' . arg_t() . ')___global_const__,(' 
		. arg_t() . ')___global_const__ + ' . const_len . ' * ___global_const_offset);
		'}}');
...
def get_fun_lib(fun_name : ptd::sim(), args : ptd::arr(ptd::sim())) 
		: ptd::sim() {
...
def get_fun_name(mod : ptd::sim(), fun_name : ptd::sim(), mod_name : ptd::sim()) : ptd::sim() {
	...
	return get_module_name(mod) . '0' . string::replace(fun_name,
		'0', '00');
}
...
def generate_imm(ref state : @generator_c::state_t, obj) : ptd::void() {
	if (nl::is_hash(obj)) {
		...
		if (ov::has_value(obj)) {
			...
			generate_imm(ref state, ov::get_value(obj));
			print(ref state, ')');
		} else {
...
def print_cmd(ref state : @generator_c::state_t, asm : @nlasm::cmd_t) : ptd::void() {
	...
	match (asm->cmd) case :arr_decl(var decl) {
		...
		var r = get_fun_lib('func_new', [
			get_fun_name(func->module, func->name, state->mod_name).'0ptr',
			get_const_sim(ref state, func->module),
			get_const_sim(ref state, func->name)
		]);
		...
		print(ref state, get_fun_lib('array_set',
			[get_reg_ref(ref state, set->src), get_reg(ref state, set->idx), get_reg(ref state, set->val)]));
	} case :get_val(var get) {
		var r = get_fun_lib('hash_get_value_dec',
			[get_reg(ref state, get->src), get_const_sim(ref state, get->key)]);
		print(ref state, get_assign(ref state, get->dest, r));
	} case :set_val(var set) {
		print(ref state, get_fun_lib('hash_set_value_dec', [
			get_reg_ref(ref state, set->src), get_const_sim(ref state, set->key), get_reg(ref state, set->val)]));
	} case :ov_mk(var mk) {
		var r;

...
def build_state(nlasms : ptd::arr(@nlasm::result_t), known_exec_func : ptd::hash(@interpreter::known_exec_func_t)) : @interpreter::state_t {
	return {
		rstate => :finished(''),
		func => {annotation => :none, access => :priv, reg_size => 0, args_type => [], commands => [], name => ''},
		labels => build_labels(nlasms),
		functions => build_functions(nlasms),
		stack => [],
		top => {
			func_key => '',
			module_name => '',
...
def interpreter::init(nlasms : ptd::arr(@nlasm::result_t), known_exec_func : ptd::hash(@interpreter::known_exec_func_t)) : @interpreter::state_t {
	return build_state(nlasms, known_exec_func);
}

def interpreter::start(ref state : @interpreter::state_t, main_fun : ptd::sim(), module_name : ptd::sim()) 
		: ptd::var({ok => ptd::sim(), err => ptd::sim()}) {
	...
	state->top = {
		...
		next => 0,
		vars => build_registers(func->reg_size),
		ret => '',
		code_vars => {},
		ref_arguments => {}
	};
...
def interpreter::evaluate_const(state : @interpreter::state_t, func : @nlasm::function_t, module : ptd::sim(),
		regs : ptd::arr(ptd::ptd_im()), ins_nr : ptd::sim())
		: ptd::var({ok => ptd::arr(ptd::ptd_im()), err => ptd::sim()}) {
...
def get_stack_element_debug(element : @interpreter::stack_element_t, state : @interpreter::state_t) : @interpreter::stack_element_debug_t {
...
def get_variables(element : @interpreter::stack_element_t) 
	: ptd::arr(ptd::rec({name=>ptd::sim(), value=>ptd::ptd_im()})) {
	var result = [];
...
def get_instruction_nr(state : @interpreter::state_t) : ptd::sim() {
...
def interpreter::execute_full_instruction(ref state : @interpreter::state_t) : ptd::void() {
	...
	state->instruction_nr = get_instruction_nr(state);
	loop {
...
def step(ref state : @interpreter::state_t) : ptd::void() {
	...
	match (command_cmd) case :arr_decl(var arr_decl) {
		...
		if (hash::has_key(state->compiler_functions, key)) {
			...
			state->rstate = :error('unknown function call: ' . key);
		}
...
def interpreter::finish_callback(fun_name : ptd::sim(), value : ptd::var({ok => ptd::sim(), err => ptd::sim()}), ref state : @interpreter::state_t) : ptd::void() {
	die unless state->rstate is :callback;
	die unless state->rstate as :callback eq fun_name;
	match (value) case :ok(var as_ok) {
		++state->top->next;
		state->top->vars[state->read_dest] = as_ok unless state->read_dest eq '';
		state->read_dest = '';
		state->rstate = :running;
		handle_new_declarations(ref state);
	} case :err(var as_err) {
...
def interpreter::finish_read(value : ptd::var({ok => ptd::sim(), err => ptd::sim()}), ref state : @interpreter::state_t) : ptd::void() {
	match (value) case :ok(var as_ok) {
		++state->top->next;
		state->top->vars[state->read_dest] = as_ok unless state->read_dest eq '';
		state->read_dest = '';
...
def handle_extern_call(call : @nlasm::call_t, ref state : @interpreter::state_t) : ptd::void() {
	...
	rep var i (array::len(args)){
		ptd::ensure_only_dynamic(func->args[i], args[i]);
	}
	var ret_val = func::exec_ref(func->func, ref args);
	match(func->type) case :sequential{
		...
		state->rstate = :callback(call->mod.'::'.call->fun_name);
		state->read_dest = call->dest;
		state->instruction_nr = -1;
		--state->top->next;
	}
}
def get_compiler_functions() : ptd::hash(ptd::sim()) {
	...
	hash::set_value(ref ret, 'array::cmp', '');
	...
	hash::set_value(ref ret, 'string::t', '');
...
def handle_array_call(key : ptd::sim(), ref ret_val : ptd::ptd_im(), ref args : ptd::arr(ptd::ptd_im()))
		: ptd::var({ok => ptd::sim(), err => ptd::sim()}) {
	...
	} elsif (key eq 'array::cmp') {
		return error_message unless nl::is_array(args[1]);
		ret_val = array::cmp(args[0], args[1]);
...
def handle_hash_call(key : ptd::sim(), ref ret_val : ptd::ptd_im(), ref args : ptd::arr(ptd::ptd_im()))
		: ptd::var({ok => ptd::sim(), err => ptd::sim()}) {
	var error_message = :err('incorrect command');
...
def handle_string_call(key : ptd::sim(), ref ret_val : ptd::ptd_im(), ref args : ptd::arr(ptd::ptd_im()))
		: ptd::var({ok => ptd::sim(), err => ptd::sim()}) {
	var error_message = :err('incorrect command');
	if (key eq 'string::lf') {
		ret_val = string::lf();
...
def handle_ov_call(key : ptd::sim(), ref ret_val : ptd::ptd_im(), ref args : ptd::arr(ptd::ptd_im()))
		: ptd::var({ok => ptd::sim(), err => ptd::sim()}) {
	var error_message = :err('incorrect command');
	if (key eq 'ov::mk') {
		return error_message unless nl::is_sim(args[0]);
...
def handle_c_rt_lib_call(key : ptd::sim(), ref ret_val : ptd::ptd_im(), ref args : ptd::arr(ptd::ptd_im()))
		: ptd::var({ok => ptd::sim(), err => ptd::sim()}) {
	var error_message = :err('incorrect command');
...
def handle_compiler_call(call : @nlasm::call_t, key : ptd::sim(), ref state : @interpreter::state_t) : ptd::void() {
	...
	if (call_result is :err) {
		state->rstate = :error((call_result as :err).key);
	} else {
		rep var arg_num(array::len(call->args)) {
			var arg = call->args[arg_num];
			match(arg) case :val(var reg) {
...
def handle_return(return_i : ptd::var({val=>@nlasm::reg_t, emp=> ptd::none()}),
		ref state : @interpreter::state_t) : ptd::void() {
